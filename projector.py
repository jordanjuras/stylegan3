...
def load_target_image(target_fname: Path, image_resolution):
    # Load target image.
    target_pil = PIL.Image.open(target_fname.as_posix()).convert('RGB')
    w, h = target_pil.size
    s = min(w, h)
    target_pil = target_pil.crop(((w - s) // 2, (h - s) // 2, (w + s) // 2, (h + s) // 2))
    target_pil = target_pil.resize((image_resolution, image_resolution), PIL.Image.LANCZOS)
    target_uint8 = np.array(target_pil, dtype=np.uint8)

    return target_uint8, target_pil
...
def run_projection_batch(
    ...
    for file_path in target_dir_path.iterdir():
        ...
        target_uint8, target_pil = load_target_image(file_path, G.img_resolution)
        ...
        original_save_path = output_dir_path / f'target_{file_ctr}.png'
        target_pil.save(f'{original_save_path.as_posix()}')
...
def run_projection(
    ...
    target_uint8, target_pil = load_target_image(target_fname, G.img_resolution)
